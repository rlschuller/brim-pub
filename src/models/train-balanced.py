...
parser.add_argument("-pw", "--pos_weight", default=1.0, type=float)
...
parser_system.add_argument("-bs", "--batch_size", default=400000, type=int)
parser_system.add_argument(
...
for epoch in range(hparams["max_epochs"]):
    ...
    for X, y, metadata in tqdm(train_dataloader, ncols=60):
        ...
        pos = (output[:,0] + output[:,1]) > 0


        input = torch.cat([input[:torch.sum(pos)*4, :], input[pos, :]])
        output = torch.cat([output[:torch.sum(pos)*4, :], output[pos, :]])
        #print()
        #print(f"{torch.sum(pos)=}")
        #print(f"{input.shape=}")
        ...
        #vi = list(range(X.shape[-1]))
        #random.shuffle(vi)

        #for i in vi:

        #    _X = X[:,:,:,:,i:i+1].to(device)
        #    _y = y[:,:,:,:,i:i+1].to(device)

        #    if hparams["optimizer"] == "manual":
        #        model.grad = None
        #    else:
        #        optimizer.zero_grad()
        ...
        #    _pred = model(_X)
        #    #print()
        #    #print(f"{torch.sum(_pred * _X[:, -1, :, :, :])=}")
        #    #print(f"{torch.sum(_pred * (1-_X[:, -1, :, :, :]))=}")

        #    #print(f"{torch.max(_X[:, -1, :, :, :])=}")
        #    #print(f"{_X.shape=}")
        #    #exit(0)
        #    loss = loss_func(_pred, _y)
        ...
        #            p.data = p.data - torch.sum(_X[:,-1,:,:,:]) * hparams["learning_rate"]*p.grad
        #    else:
        #        optimizer.step()
    ...
    true_positive = 0
    with torch.no_grad():
        for X, y, _ in tqdm(validation_dataloader, ncols=60):
            X = X#.to(device)
            y = y#.to(device)
            ...
            for i in range(exam.shape[-2]):
                ...
                _X = exam[:,:,:,i,:].to(device)
                _y = y[:,:,:,i,:].to(device)
                _pred_mask = torch.cat([mask[:,:,:,i], mask[:,:,:,i]],dim=0).permute(1,2,0).to(device)
