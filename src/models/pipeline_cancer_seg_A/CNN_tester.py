...
parser.add_argument(
    "-output_path",
    default = './data/processed/nrrd_segs/testes',
    type=str
)

parser.add_argument(
    "-model_dict",
    default = './models/mass_seg/last_trained_model.pth',
    type=str
)
...
#iniciar o modelo
model = CNN_test().to(device=device)
...
def apply_model_in(exam_path, fold, output_path, cube_size = 16):
    ...
    At = len(range(0, data.shape[2]-cube_size, cube_size//2)) * len(range(0, data.shape[3]-cube_size, cube_size//2)) * len(range(0, data.shape[4]-cube_size, cube_size//2))
    Ac = 0
        ...
        for j in range(0, data.shape[3]-cube_size, cube_size//2):
            for k in range(0, data.shape[4]-cube_size, cube_size//2):
                ...
                if 1 in seg[i:i+cube_size, j:j+cube_size, k:k+cube_size]:
                        ...
                        # if score[0][2] > 0.4:
                        #     cancers[i:i+cube_size, j:j+cube_size, k:k+cube_size] = 4
                        # elif score[0][1] > 0.5:
                        #     cancers[i:i+cube_size, j:j+cube_size, k:k+cube_size] = 3
    ...
    nrrd.write(f'{output_path}/{fold}_cancers.seg.nrrd', cancers, meta)
